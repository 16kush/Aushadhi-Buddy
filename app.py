...
from nlp.nnet import NeuralNet
from nlp.nltk_utils import bag_of_words
from flask import Flask, render_template, url_for, request, jsonify

data_dir ="E:/nltk_data/packages/"
...
diseases_description = pd.read_csv("data/symptom_des.csv")
...
with open('data/symptoms.pickle', 'rb') as data_file:
    symptoms_list = pickle.load(data_file)

with open('models/fit_model.pickle', 'rb') as modelFile:
    ...
    if sentence.replace(".", "").replace("!","").lower().strip() == "done":
        ...
        if not user_symptoms:
            response_sentence = random.choice(
                ...
                "Aushadhi Buddy can't know the disease if there are no symptoms...",
            ...
            if np.mean(severity) > 4 or np.max(severity) > 5:
                response_sentence = response_sentence + "<br><br>Considering your symptoms are severe, and Aushadhi Buddy isn't a real doctor, you should consider talking to one. :)"
